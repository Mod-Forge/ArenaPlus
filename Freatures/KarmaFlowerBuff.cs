...
{
    internal static class KarmaFlowerBuff
    {
        internal static void OnEnable()
        {
            logSource.LogInfo("KarmaFlowerBuff OnEnable");
        ...
        private static void PlayerGraphics_AddToContainer(On.PlayerGraphics.orig_AddToContainer orig, PlayerGraphics self, RoomCamera.SpriteLeaser sLeaser, RoomCamera rCam, FContainer newContatiner)
        {
            ...
            if (rCam.room.game.IsArenaSession && !IsChallengeGameSession(rCam.room.game) && rCam.room.game.rainWorld.progression.miscProgressionData.beaten_Saint && self.player.SlugCatClass != MoreSlugcatsEnums.SlugcatStatsName.Saint)
            {
        ...
        private static void PlayerGraphics_DrawSprites(On.PlayerGraphics.orig_DrawSprites orig, PlayerGraphics self, RoomCamera.SpriteLeaser sLeaser, RoomCamera rCam, float timeStacker, UnityEngine.Vector2 camPos)
        {
            ...
            if (rCam.room.game.IsArenaSession && !IsChallengeGameSession(rCam.room.game) && rCam.room.game.rainWorld.progression.miscProgressionData.beaten_Saint)
            {
                PlayerCustomData customData = (self.owner as Player).GetCustomData<PlayerCustomData>();
        ...
        {
            if (self.room.game.IsArenaSession && !IsChallengeGameSession(self.room.game) && self.room.game.rainWorld.progression.miscProgressionData.beaten_Saint && self.bites < 2)
            {
